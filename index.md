---
title: Home
layout: home
nav_order: 0
---

# Sanghyeon Tech Wiki

비전공자에서 인프라 엔지니어로.
서버, 네트워크, 클라우드부터 자동화, 웹개발까지 매일 배우고 부딪히고 정리하는 기록입니다.

## 학습 트랙

- [01. 네트워크]({{ site.baseurl }}/01-network/)
- [02. 컴퓨터구조]({{ site.baseurl }}/02-computer-architecture/)
- [03. 리눅스]({{ site.baseurl }}/03-linux/)
- [04. 윈도우]({{ site.baseurl }}/04-windows/)
- [05. 자동화 (Python/Shell)]({{ site.baseurl }}/05-automation-python-shell/)
- [06. 보안]({{ site.baseurl }}/06-security/)
- [07. AWS]({{ site.baseurl }}/07-aws/)
- [08. 가상화/컨테이너/오케스트레이션]({{ site.baseurl }}/08-virtualization-docker/)
- [09. 모니터링 & 백업]({{ site.baseurl }}/09-monitoring-backup/)
- [10. Git]({{ site.baseurl }}/10-git/)
- [11. HTML/CSS/JS]({{ site.baseurl }}/11-html-css-js/)
- [12. Java/Spring]({{ site.baseurl }}/12-java-spring/)
- [13. SQL]({{ site.baseurl }}/13-sql/)
- [14. CI/CD]({{ site.baseurl }}/14-cicd/)

## 자격증

- [CCNA / RHCSA / AWS SAA]({{ site.baseurl }}/certifications/)

## 회고

- [15. 회고]({{ site.baseurl }}/15-retrospective/)
